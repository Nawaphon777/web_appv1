            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedItem.dep" label="Department"></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
...
export default {
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      ...
      const url = 'http://localhost:9000/delete?id=' + this.editedItem.id;
      const res = await fetch(url);
      const data = await res.json();
    ...
    async save() {
      if (this.editedIndex > -1) {
        const res = await axios.post('http://localhost:9000/edit', this.editedItem);
        console.log('data=', this.editedItem);
      } else {
        this.desserts.push(this.editedItem);
        const res = await axios.post('http://localhost:9000/add', this.editedItem);
        console.log('data=', this.editedItem);
      }
      this.close();
    },
